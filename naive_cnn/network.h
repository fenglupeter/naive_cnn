...
#ifndef NAIVE_CNN_NETWORK_H_
#define NAIVE_CNN_NETWORK_H_
...
#include "naive_cnn/layers/layers.h"
...
struct result {
    ...
    void print_summary(std::basic_ostream<Char, CharTraits>& os) const {
        os << "accuracy:" << accuracy() << "% (" << num_success
        << "/" << num_total << ")" << std::endl;
    }
...
public:
  typedef LossFunction E;
     ...
     * executes forward-propagation and returns output
     *
     * @param in input value range(double[],
     * std::vector<double>, std::list<double> etc)
     **/
     ...
     * @param t_cost             target costs (leave to nullptr
     * in order to assume equal cost for every target)
     */
        ...
        for (int iter = 0; iter < epoch; iter++) {
            ...
            for (size_t i = 0; i < in.size(); i+=batch_size) {
                ...
                if (i % 100 == 0 && layers_.is_exploded()) {
                    std::cout << "[Warning]Detected infinite value in weight. "
                                 << "stop learning." << std::endl;
                    return false;
                }
            }
            on_epoch_enumerate();
    ...
    template<typename T>
    bool train(const std::vector<vec_t>& in,
        const std::vector<T>& t, size_t batch_size = 1, int epoch = 1) {
        set_netphase(net_phase::train);
    ...
    void save(std::ostream& os) const {
        os.precision(std::numeric_limits<naive_cnn::float_t>::digits10);
    ...
    void load(std::istream& is) {
        is.precision(std::numeric_limits<naive_cnn::float_t>::digits10);
     ...
     **/
    bool gradient_check(const vec_t* in, const label_t* t, int data_size,
        ...
        while ((current = current->next()) != 0) { // ignore first input layer
            ...
            case GRAD_CHECK_ALL:
                for (int i = 0; i < (int)w.size(); i++)
                    if (!calc_delta(in, &v[0], data_size, w, dw, i, eps))
                        return false;
                for (int i = 0; i < (int)b.size(); i++)
                    if (!calc_delta(in, &v[0], data_size, b, db, i, eps))
                        return false;
                break;
            case GRAD_CHECK_RANDOM:
                for (int i = 0; i < 10; i++)
                    if (!calc_delta(in, &v[0], data_size, w, dw,
                        uniform_idx(w), eps))
                        return false;
                for (int i = 0; i < 10; i++)
                    if (!calc_delta(in, &v[0], data_size, b, db,
                        uniform_idx(b), eps))
                        return false;
                break;
            default:
...
private:
    ...
    void label2vector(const label_t* t, int num,
    ...
    void train_once(const vec_t* in, const label_t* t, int size,
        const int nbThreads, const vec_t* t_cost) {
        std::vector<vec_t> v;
    ...
    /** 
     * trains on one minibatch, i.e. runs forward and backward propagation
     * to calculate
    ...
    void train_onebatch(const vec_t* in, const vec_t* t, int batch_size,
        const int num_tasks, const vec_t* t_cost) {
        ...
        for_i(num_threads, [&](int i) {
            ...
            for (int j = start_index; j < end_index; ++j)
                bprop(fprop(in[j], i), t[j], i, t_cost ?
                &(t_cost[j]) : nullptr);
        }, 1);
    ...
    void calc_hessian(const std::vector<vec_t>& in,
        const std::vector<vec_t>* t_cost, int size_initialize_hessian = 500) {
     ...
     * @return true if the combination of the loss function E and the last
     * layer output activation
     *         function h is such that dE / da = (dE/dY) * (dy/da) = y - target
     */
    template<typename Activation>
    bool is_canonical_link(const Activation& h) {
        if (typeid(h) == typeid(activation::sigmoid) &&
        typeid(E) == typeid(cross_entropy)) return true;
        if (typeid(h) == typeid(activation::tan_h)
        && typeid(E) == typeid(cross_entropy)) return true;
        if (typeid(h) == typeid(activation::identity)
        && typeid(E) == typeid(mse)) return true;
        if (typeid(h) == typeid(activation::softmax)
        && typeid(E) == typeid(cross_entropy_multiclass)) return true;
        return false;
    }
    ...
    void bprop_2nd(const vec_t& out, const vec_t* t_cost) {
        ...
        if (is_canonical_link(h)) {
            for_i(out_dim(), [&](int i){ delta[i] = target_value_max() *
            h.df(out[i]);});
        } else {
            for_i(out_dim(), [&](int i){ delta[i] = target_value_max() *
            h.df(out[i]) * h.df(out[i]);}); // FIXME
        }
    ...
    bool calc_delta(const vec_t* in, const vec_t* v, int data_size, vec_t& w,
        vec_t& dw, int check_index, double eps) {
        ...
        for(int i = 0; i < data_size; i++) {
            f_p += get_loss(fprop(in[i]), v[i]);
        }

        float_t f_m = float_t(0);
        w[check_index] = prev_w - delta;
        for(int i = 0; i < data_size; i++) {
            f_m += get_loss(fprop(in[i]), v[i]);
        }
    ...
    void check_t(size_t i, label_t t, cnn_size_t dim_out) {
        if (t >= dim_out) {
            ...
            os << format_str("t[%u]=%u, dim(network output)=%u", i, t, dim_out)
            << std::endl;
            os << "in classification task, dim(network output)"
                  << " must be greater than max class id." << std::endl;
            if (dim_out == 1)
                os << std::endl << "(for regression, use vector<vec_t> instead "
                                   << "of vector<label_t> for training signal)"
                                   << std::endl;
    ...
    void check_training_data(const std::vector<vec_t>& in,
        ...
        if (in.size() != t.size())
            throw nn_error("number of training "
                           "data must be equal to label data");
    ...
    template <typename T>
    void check_target_cost_matrix(const std::vector<T>& t,
    ...
    void check_target_cost_element(const label_t t, const vec_t& t_cost) {
        if (t >= t_cost.size()) {
            throw nn_error("if target cost is supplied "
                           "for a classification task, some cost must be "
                           "given for each distinct class label");
        }
    }
    ...
    void check_target_cost_element(const vec_t& t, const vec_t& t_cost) {
        if (t.size() != t_cost.size()) {
            throw nn_error("if target cost is supplied "
                           "for a regression task, its shape must be "
                           "identical to the target data");
        }
    }

    inline const vec_t* get_target_cost_sample_pointer(
        onst std::vector<vec_t>* t_cost, size_t i) {
    ...
    float_t target_value_min() const { return layers_.tail()
    ->activation_function().scale().first; }
    float_t target_value_max() const { return layers_.tail()
    ->activation_function().scale().second; }
...
{
    assert(step>0);
    std::vector<vec_t> res((cols - sizepatch)* (rows - sizepatch) /
    (step * step),vec_t(sizepatch * sizepatch));
...
network<L, O>& operator << (network<L, O>& n, Layer&& l) {
    n.add(std::make_shared<typename std::remove_reference<Layer>::type>
        (std::forward<Layer>(l)));
    return n;
}
...
std::basic_ostream<Char, CharTraits>& operator <<
    (std::basic_ostream<Char, CharTraits>& os, const network<L, O>& n) {
    n.save(os);
    return os;
}
...
(std::basic_istream<Char, CharTraits>& os, network<L, O>& n) {
    n.load(os);
    return os;
}
